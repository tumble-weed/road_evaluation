...
from analyze_results import read_json_and_flatten
##
# some functions
def load_scores(jsons_dict):
    flat = {}
    for retrain_status,j in json_dict.items():
        j_flat = read_json_and_flatten(j)
        flat.update(
            {((retrain_status,) + k):v for k,v in j_flat.items()}
        )
    return flat
def rank_corr_avg(flat):
    # key format: (retrain_status,imputation,base_method,modifier,order,perc)
    run_avg = {}
    import ipdb;ipdb.set_trace()
    run_avg = {k:np.mean(v,axis=0) for k,v in flat.items()}
    # combine the base_method and modifier keys:
    run_avg = { (retrain_status,imputation,base_method+'-'+modifier,order,perc):v for (retrain_status,imputation,base_method,modifier,order,perc),v in run_avg.items()}
    percentages = list(set([perc for (retrain_status,imputation,methods,order,perc),v in run_avg.items()]))
    # retrain_statuses = list(set([retrain_status for (retrain_status,imputation,methods,order,perc),v in run_avg.items()]))
    # imputations = list(set([imputation for (retrain_status,imputation,methods,order,perc),v in run_avg.items()]))
    
    # orders = list(set([order for (retrain_status,imputation,methods,order,perc),v in run_avg.items()]))
    except_percentage = list(set([(retrain_status,imputation,method,order) for (retrain_status,imputation,method,order,perc),v in run_avg.items()]))
    perc_avg = {}
    for other in except_percentage:
        perc_avg[other] = np.mean([run_avg[other + (perc,)] for perc in percentages])
    methods = list(set([method for (retrain_status,imputation,method,order),v in run_avg.items()]))
    except_method = list(set([(retrain_status,imputation,order) for (retrain_status,imputation,method,order),v in run_avg.items()]))
    for other in except_method:
        perc_avg[other] = np.array([run_avg[tuple(list(other).insert(method,2))] for method in methods]).argsort(axis=0)
    
    # perc_avg size is retrain_status,imputation,order
    import ipdb;ipdb.set_trace()
            
        
    
    # run_avg shape is (retrain_status,imputation,method,order,perc)
if False:
